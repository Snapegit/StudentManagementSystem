		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="考勤类型" prop="kaoqinleixing">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.kaoqinleixing?true:false"
								v-model="form.kaoqinleixing" 
								placeholder="请选择考勤类型"
								>
								<el-option v-for="(item,index) in kaoqinleixingLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="12">
						<el-form-item label="第几周" prop="dijizhou">
							<el-input class="list_inp" v-model="form.dijizhou" placeholder="第几周"
								 type="text" 								:readonly="!isAdd||disabledForm.dijizhou?true:false" />
						</el-form-item>
					</el-col>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='kaoqinleixing'){
					form.value.kaoqinleixing = row[x];
					disabledForm.value.kaoqinleixing = true;
					continue;
				}
				if(x=='dijizhou'){
					form.value.dijizhou = row[x];
					disabledForm.value.dijizhou = true;
					continue;
				}
				if(x=='dijijie'){
		...
		//由上级字段带出不可改
		disabledForm.value.banji = true;
		kaoqinleixingLists.value = "出勤,迟到,早退,旷课".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
